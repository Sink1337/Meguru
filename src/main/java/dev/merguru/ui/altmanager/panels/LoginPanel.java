...
import dev.merguru.utils.text.RandomStringGenerator;
...
public class LoginPanel extends Panel {
        ...
        for (ActionButton actionButton : actionButtons) {
            ...
            actionButton.setClickAction(() -> {
                switch (actionButton.getName()) {
                    case "Login":
                        Merguru.INSTANCE.getAltManager().getUtils().login(textFields.get(0), textFields.get(1));
                        resetTextFields();
                        Merguru.INSTANCE.getAltManager().getAltPanel().refreshAlts();
                        break;
                    case "Add":
                        TextField username = textFields.get(0);
                        ...
                        break;
                    case "Gen Cracked":
                        Merguru.INSTANCE.getAltManager().getUtils().loginWithString(generator.generate(8), "", false);
                        Merguru.INSTANCE.getAltManager().getAltPanel().refreshAlts();
                        break;
                }
            });
    ...
    public void mouseClicked(int mouseX, int mouseY, int button) {
        ...
        if (hoveringMicrosoft && button == 0) {
            ...
            Merguru.INSTANCE.getAltManager().getUtils().microsoftLoginAsync(email, password);
            resetTextFields();
        }
