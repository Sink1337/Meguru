...
public class LocalScriptRect implements Screen {
    ...
    public LocalScriptRect(Script script) {
        this.script = script;
        Merguru.INSTANCE.getSideGui().addTooltip(hoverInformation);
            ...
            button.setClickAction(() -> {
                ...
                switch (button.getIcon()) {
                    case FontUtil.UPLOAD:
                        Form uploadForm = Merguru.INSTANCE.getSideGui().displayForm("Upload Script");
                        uploadForm.setUploadAction((name, description) -> {
                            Multithreading.runAsync(() -> {
                                boolean validScript = Merguru.INSTANCE.getScriptManager().processScriptData(script.getFile());
                                if (validScript) {
