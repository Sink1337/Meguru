...
import dev.merguru.ui.sidegui.forms.impl.EditForm;
import dev.merguru.ui.sidegui.utils.CloudDataUtils;
import dev.merguru.ui.sidegui.utils.IconButton;
import dev.merguru.ui.sidegui.utils.TooltipObject;
import dev.merguru.ui.sidegui.utils.VoteRect;
...
public class CloudConfigRect implements Screen {
    ...
    public CloudConfigRect(CloudConfig config) {
        ...
        Merguru.INSTANCE.getSideGui().getTooltips().add(hoverInformation);
                ...
                switch (iconButton.getIcon()) {
                    case FontUtil.LOAD:
                        Multithreading.runAsync(() -> {
                            ...
                            if (Merguru.INSTANCE.getConfigManager().loadConfig(loadData, false)) {
                    ...
                    case FontUtil.SAVE:
                        Multithreading.runAsync(() -> {
                            ...
                            if (Merguru.INSTANCE.getConfigManager().saveConfig(name, saveData)) {
                            ...
                            Merguru.INSTANCE.getSideGui().getTooltips().clear();
                            Merguru.INSTANCE.getSideGui().getConfigPanel().setRefresh(true);
                        });
                        break;
                    case FontUtil.EDIT:
                        Form form = Merguru.INSTANCE.getSideGui().displayForm("Edit Config");
                        ((EditForm) form).setup(config, false);
                        form.setUploadAction((fileName, updatedDescription) -> {
                            Multithreading.runAsync(() -> {

                                String data = Merguru.INSTANCE.getConfigManager().serialize();
