package dev.merguru.ui.sidegui.panels.configpanel;

import dev.merguru.Merguru;
import dev.merguru.config.ConfigManager;
import dev.merguru.config.LocalConfig;
import dev.merguru.intent.cloud.CloudUtils;
import dev.merguru.intent.cloud.data.CloudConfig;
import dev.merguru.intent.cloud.data.Votes;
import dev.merguru.module.impl.render.HUDMod;
import dev.merguru.ui.sidegui.SideGUI;
import dev.merguru.ui.sidegui.forms.Form;
import dev.merguru.ui.sidegui.panels.Panel;
import dev.merguru.ui.sidegui.utils.ActionButton;
import dev.merguru.ui.sidegui.utils.CarouselButtons;
import dev.merguru.ui.sidegui.utils.DropdownObject;
import dev.merguru.ui.sidegui.utils.ToggleButton;
import dev.merguru.utils.misc.Multithreading;
import dev.merguru.utils.objects.Scroll;
...
public class ConfigPanel extends Panel {
    ...
    public void drawScreen(int mouseX, int mouseY) {
        ...
        if (voteRefreshTimer.hasTimeElapsed(30000)) {
            Multithreading.runAsync(() -> Merguru.INSTANCE.getCloudDataManager().refreshVotes());
            voteRefreshTimer.reset();
        }


        Merguru.INSTANCE.getCloudDataManager().applyVotes();
        ...
        for (ActionButton button : actionButtons) {
            ...
            button.setClickAction(() -> {
                switch (button.getName()) {
                    case "Upload config":
                        Form uploadForm = Merguru.INSTANCE.getSideGui().displayForm("Upload Config");
                        uploadForm.setTriUploadAction((name, description, server) -> {
                            Multithreading.runAsync(() -> {
                                String fileData = Merguru.INSTANCE.getConfigManager().serialize();
                                CloudUtils.postOnlineConfig(name, description, server, fileData);
                                Merguru.INSTANCE.getCloudDataManager().refreshData();
                            });
                        });
                        break;
                    case "Save current config":
                        Form saveForm = Merguru.INSTANCE.getSideGui().displayForm("Save Config");
                        saveForm.setUploadAction((name, description) -> {
                            Multithreading.runAsync(() -> {
                                Merguru.INSTANCE.getConfigManager().saveConfig(name);
                                Merguru.INSTANCE.getCloudDataManager().refreshData();
                            });
                        });
                        break;
        ...
        switch (carouselButtons.getCurrentButton()) {
            case "Cloud":
                ...
                if (!sortingSelection.equals(sorting.getSelection())) {
                    Multithreading.runAsync(() -> Merguru.INSTANCE.getCloudDataManager().refreshData());
                    sortingSelection = sorting.getSelection();
                }

                if (Merguru.INSTANCE.getCloudDataManager().isRefreshing())
                    return;
                ...
                for (CloudConfigRect cloudConfigRect : cloudConfigRects) {
                    ...
                    if (filterAuthor) {
                        ...
                        if (!Merguru.INSTANCE.getIntentAccount().username.equals(configAuthor)) continue;
                    }

                    if (filterVersion && !cloudConfigRect.getConfig().getVersion().equals(Merguru.VERSION)) continue;
    ...
    public void refresh() {
        ...
        for (CloudConfig cloudConfig : Merguru.INSTANCE.getCloudDataManager().getCloudConfigs()) {
    ...
    public Comparator<CloudConfigRect> relevanceSorting() {
        return Comparator.<CloudConfigRect>comparingInt(configRect -> {
            ...
            versionScore = cloudConfig.getVersion().equals(Merguru.VERSION) ? 100 : 0;
