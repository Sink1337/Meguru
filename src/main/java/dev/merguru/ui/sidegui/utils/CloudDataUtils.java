...
public class CloudDataUtils {
    ...
    public static void refreshCloud() {
        if (Merguru.INSTANCE.getSideGui().getPanels() != null) {
            ConfigPanel configPanel = (ConfigPanel) Merguru.INSTANCE.getSideGui().getPanels().get("Configs");
            configPanel.refresh();

            ScriptPanel scriptPanel = (ScriptPanel) Merguru.INSTANCE.getSideGui().getPanels().get("Scripts");
            scriptPanel.refresh();
        }
