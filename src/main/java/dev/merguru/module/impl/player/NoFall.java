...
import dev.merguru.utils.player.PlayerUtils;
...
public final class NoFall extends Module {
    ...
    public void onMotionEvent(MotionEvent event) {
        if (event.isPre()) {
            ...
            if (mode.is("BlocksMC")) {
                if (mc.thePlayer.fallDistance > 3.5 && !PlayerUtils.overVoid() && Merguru.INSTANCE.getModuleCollection().getModule(KillAura.class).isEnabled()) {
