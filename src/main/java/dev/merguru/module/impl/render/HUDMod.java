...
import dev.merguru.module.impl.player.InvManager;
import dev.merguru.module.settings.ParentAttribute;
import dev.merguru.module.settings.impl.*;
...
import dev.merguru.utils.misc.RomanNumeralUtils;
import dev.merguru.utils.player.MovementUtils;
import dev.merguru.utils.render.*;
...
public class HUDMod extends Module {
    ...
    public void onShaderEvent(ShaderEvent e) {
        ...
        if (e.isBloom()) {
            ...
            String intentInfo = Merguru.INSTANCE.getIntentAccount().username;
            float WH = 110 / 2f;
            switch (watermarkMode.getMode()) {
                ...
                case "Tenacity":
                    float xVal = 6f;
                    float yVal = 6f;
                    float versionWidth = FontUtil.tenacityFont16.getStringWidth(Merguru.INSTANCE.getVersion());
                    ...
                    GradientUtil.applyGradientHorizontal(xVal, yVal, width, 20, 1, clientColors.getFirst(), clientColors.getSecond(), () -> {
                        ...
                        if (version) {
                            FontUtil.tenacityFont16.drawString(Merguru.INSTANCE.getVersion(), versionX, yVal, 0);
                        }
                    });
    ...
    public void onRender2DEvent(Render2DEvent e) {
        ...
        Pair<Color, Color> clientColors = getClientColors();
        String name = Merguru.NAME;
        PostProcessing postProcessing = Merguru.INSTANCE.getModuleCollection().getModule(PostProcessing.class);
        ...
        version = name.equalsIgnoreCase(Merguru.NAME);
        ...
        switch (watermarkMode.getMode()) {
            ...
            case "Tenacity":
                ...
                GradientUtil.applyGradientHorizontal(xVal + spacing, yVal + spacing, width + spacing,
                        20, 1, darkerColors.getFirst(), darkerColors.getSecond(), () -> {
                            ...
                            if (version) {
                                FontUtil.tenacityFont16.drawString(Merguru.INSTANCE.getVersion(), versionX + (spacing / 2f), yVal + (spacing / 2f), 0);
                            }
                        });
            ...
            case "Exhi1":
                ...
                if (watermarkCustomization.getSetting("Client Version").isEnabled()) {
                    stringBuilder.append(" [§f").append(Merguru.VERSION).append("§7]");
                }
            ...
            case "Exhi2" :
                ...
                if (watermarkCustomization.getSetting("Client Version").isEnabled()) {
                    stringBuilder1.append(" [§f").append(Merguru.VERSION).append("§7]");
                }
                if (watermarkCustomization.getSetting("Time").isEnabled()) {
    ...
    private void drawBottomRight() {
        ...
        if (infoCustomization.isEnabled("Show Bottom Right Text")) {
            String text = Merguru.VERSION + " - " + (customFont.isEnabled() ? "" : "§l") + Merguru.RELEASE.getName() + "§r";

            text += " - " + Merguru.INSTANCE.getIntentAccount().username;
            text += " (" + Merguru.INSTANCE.getIntentAccount().client_uid + ")";
    ...
    public static ModeSetting getCustomFontMode() {
        return ((HUDMod) Merguru.INSTANCE.getModuleCollection().getModule(HUDMod.class)).customFontMode;
    }
    ...
    public static void updateButtonStatus() {
        for (ModuleButton mb : ModuleButton.values()) {
            mb.getButton().enabled = Merguru.INSTANCE.getModuleCollection().getModule(mb.getModule()).isEnabled();
        }
    }
    ...
    public static void handleActionPerformed(GuiButton button) {
        for (ModuleButton mb : ModuleButton.values()) {
            if (mb.getButton() == button) {
                Module m = Merguru.INSTANCE.getModuleCollection().getModule(mb.getModule());
                if (m.isEnabled()) {
                    m.toggle();
                }
                break;
