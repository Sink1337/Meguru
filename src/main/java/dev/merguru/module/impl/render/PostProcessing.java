...
import dev.merguru.ui.clickguis.modern.ModernClickGui;
import dev.merguru.utils.render.RenderUtil;
import dev.merguru.utils.render.blur.KawaseBloom;
import dev.merguru.utils.render.blur.KawaseBlur;
...
public class PostProcessing extends Module {
    ...
    public void stuffToBlur(boolean bloom) {
        ...
        if (mc.currentScreen == ClickGUIMod.dropdownClickGui || mc.currentScreen == ClickGUIMod.modernClickGui ) {
            Merguru.INSTANCE.getSideGui().drawForEffects(bloom);
            Merguru.INSTANCE.getSearchBar().drawEffects();
        }
        ...
        RenderUtil.resetColor();
        NotificationsMod notificationsMod = Merguru.INSTANCE.getModuleCollection().getModule(NotificationsMod.class);
    ...
    public void blurScreen() {
        ...
        if (blur.isEnabled()) {
            ...
            Merguru.INSTANCE.getEventProtocol().handleEvent(new ShaderEvent(false, glowOptions));
            stuffToBlur(false);
        ...
        if (bloom.isEnabled()) {
            ...
            Merguru.INSTANCE.getEventProtocol().handleEvent(new ShaderEvent(true, glowOptions));
            stuffToBlur(true);
