...
import dev.merguru.commands.impl.FriendCommand;
...
public final class KillAura extends Module {
        ...
        if (event.isPost()) {
            if (TargetManager.target != null) {
                if (attackTimer.hasTimeElapsed(cps, true)) {
                    ...
                    Merguru.INSTANCE.getEventProtocol().handleEvent(attackEvent);
                    attack();
                }
            }
        }
    ...
    public void onMotionEvent(MotionEvent event) {
        ...
        if (autoblock.isEnabled() && InventoryUtils.isHoldingSword() && autoblockMode.is("BlocksMC B") && !targets.isEmpty() && !Merguru.INSTANCE.isEnabled(Scaffold.class)) {
            if (event.isPre()) {
        ...
        if (event.isPre()) {
            attacking = !targets.isEmpty() && (addons.getSetting("Allow Scaffold").isEnabled() || !Merguru.INSTANCE.isEnabled(Scaffold.class));
                    ...
                    if (rotations.isEnabled()) {
                        ...
                        switch (rotationMode.getMode()) {
                            ...
                            case "Smooth":
                                ...
                                break;
                            case "HVH":
                                rotations = dev.merguru.utils.addons.rise.RotationUtils.getHVHRotation(TargetManager.target);
                                break;
                            case "BlocksMC":
                                rotations = RotationUtils.getRotationsNeeded(TargetManager.target);
