...
import dev.merguru.config.ConfigSetting;
...
public class Module extends ListenerAdapter implements Utils {
    ...
    public void onEnable() {
        Merguru.INSTANCE.getEventProtocol().register(this);
    }

    public void onDisable() {
        if (this instanceof GlowESP) {
            GlowESP.fadeIn.setDirection(Direction.BACKWARDS);
            Multithreading.schedule(() -> Merguru.INSTANCE.getEventProtocol().unregister(this), 250, TimeUnit.MILLISECONDS);
        } else {
            Merguru.INSTANCE.getEventProtocol().unregister(this);
        }
    }
