...
import dev.meguru.intent.cloud.data.CloudConfig;
import dev.meguru.intent.cloud.data.Votes;
import dev.meguru.module.impl.render.HUDMod;
import dev.meguru.ui.sidegui.SideGUI;
import dev.meguru.ui.sidegui.forms.Form;
import dev.meguru.ui.sidegui.panels.Panel;
import dev.meguru.ui.sidegui.utils.ActionButton;
import dev.meguru.ui.sidegui.utils.CarouselButtons;
import dev.meguru.ui.sidegui.utils.DropdownObject;
import dev.meguru.ui.sidegui.utils.ToggleButton;
...
public class ConfigPanel extends Panel {
    ...
    public void drawScreen(int mouseX, int mouseY) {
        ...
        if (voteRefreshTimer.hasTimeElapsed(30000)) {
            Multithreading.runAsync(() -> Meguru.INSTANCE.getCloudDataManager().refreshVotes());
            voteRefreshTimer.reset();
        }


        Meguru.INSTANCE.getCloudDataManager().applyVotes();
            ...
            button.setClickAction(() -> {
                switch (button.getName()) {
                    case "Upload config":
                        ...
                        uploadForm.setTriUploadAction((name, description, server) -> {
                            Multithreading.runAsync(() -> {
                                String fileData = Meguru.INSTANCE.getConfigManager().serialize();
                    ...
                    case "Save current config":
                        Form saveForm = Meguru.INSTANCE.getSideGui().displayForm("Save Config");
                        saveForm.setUploadAction((name, description) -> {
                            Multithreading.runAsync(() -> {
                                Meguru.INSTANCE.getConfigManager().saveConfig(name);
                                Meguru.INSTANCE.getCloudDataManager().refreshData();
                            });
                        });
                        break;
        ...
        switch (carouselButtons.getCurrentButton()) {
            case "Cloud":
                ...
                if (!sortingSelection.equals(sorting.getSelection())) {
                    Multithreading.runAsync(() -> Meguru.INSTANCE.getCloudDataManager().refreshData());
                    sortingSelection = sorting.getSelection();
                }

                if (Meguru.INSTANCE.getCloudDataManager().isRefreshing())
                    return;
                ...
                for (CloudConfigRect cloudConfigRect : cloudConfigRects) {
                    ...
                    if (filterAuthor) {
                        ...
                        if (!Meguru.INSTANCE.getIntentAccount().username.equals(configAuthor)) continue;
                    }

                    if (filterVersion && !cloudConfigRect.getConfig().getVersion().equals(Meguru.VERSION)) continue;
    ...
    public void refresh() {
        ...
        for (CloudConfig cloudConfig : Meguru.INSTANCE.getCloudDataManager().getCloudConfigs()) {
    ...
    public Comparator<CloudConfigRect> relevanceSorting() {
        return Comparator.<CloudConfigRect>comparingInt(configRect -> {
            ...
            versionScore = cloudConfig.getVersion().equals(Meguru.VERSION) ? 100 : 0;
