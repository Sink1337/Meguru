...
public class LocalScriptRect implements Screen {
    ...
    public LocalScriptRect(Script script) {
        this.script = script;
        Meguru.INSTANCE.getSideGui().addTooltip(hoverInformation);
            ...
            button.setClickAction(() -> {
                ...
                switch (button.getIcon()) {
                    case FontUtil.UPLOAD:
                        Form uploadForm = Meguru.INSTANCE.getSideGui().displayForm("Upload Script");
                        uploadForm.setUploadAction((name, description) -> {
                            Multithreading.runAsync(() -> {
                                boolean validScript = Meguru.INSTANCE.getScriptManager().processScriptData(script.getFile());
                                if (validScript) {
