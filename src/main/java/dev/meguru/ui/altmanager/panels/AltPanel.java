package dev.meguru.ui.altmanager.panels;

import dev.meguru.Meguru;
import dev.meguru.ui.Screen;
import dev.meguru.ui.altmanager.Panel;
import dev.meguru.ui.altmanager.helpers.Alt;
import dev.meguru.ui.altmanager.helpers.AltManagerUtils;
...
import dev.meguru.utils.objects.DoubleIconButton;
import dev.meguru.utils.objects.Scroll;
import dev.meguru.utils.objects.TextField;
import dev.meguru.utils.render.*;
import dev.meguru.utils.server.ban.HypixelBan;
...
public class AltPanel extends Panel {
    ...
    public void keyTyped(char typedChar, int keyCode) {
        if (Meguru.INSTANCE.getAltManager().isTyping()) return;
    ...
    public void drawScreen(int mouseX, int mouseY) {
        ...
        TextField searchField = Meguru.INSTANCE.getAltManager().searchField;
        boolean filterBanned = Meguru.INSTANCE.getAltManager().filterBanned.isEnabled();
    ...
    public static class AltRect implements Screen {
        ...
        public void drawScreen(int mouseX, int mouseY) {
            ...
            switch (alt.getType()) {
                ...
                case "Mojang":
                    ...
                    mc.getTextureManager().bindTexture(new ResourceLocation("meguru/mojang.png"));
                    Gui.drawModalRectWithCustomSizedTexture(typeX + 2, usernameY - 1, 0, 0, size, size, size, size);
                    RenderUtil.resetColor();
                    break;
                case "Cracked":
                    Color red = Meguru.INSTANCE.getSideGui().getRedBadColor();
            ...
            if (Meguru.INSTANCE.getAltManager().currentSessionAlt == alt) {
                Color green = Meguru.INSTANCE.getSideGui().getGreenEnabledColor();
        ...
        public void drawAltHead(float x, float y, float size) {
            Meguru.INSTANCE.getAltManager().getUtils().getHead(alt);
            GLUtil.startBlend();
            RenderUtil.setAlphaLimit(0);
            mc.getTextureManager().bindTexture(alt.head != null ? alt.head : new ResourceLocation("meguru/X-Steve.png"));
