...
import dev.meguru.utils.text.RandomStringGenerator;
...
public class LoginPanel extends Panel {
    ...
    public void drawScreen(int mouseX, int mouseY) {
        ...
        for (ActionButton actionButton : actionButtons) {
            ...
            actionButton.setClickAction(() -> {
                switch (actionButton.getName()) {
                    case "Login":
                        Meguru.INSTANCE.getAltManager().getUtils().login(textFields.get(0), textFields.get(1));
                        resetTextFields();
                        Meguru.INSTANCE.getAltManager().getAltPanel().refreshAlts();
                        break;
                    case "Add":
                        TextField username = textFields.get(0);
                        ...
                        break;
                    case "Gen Cracked":
                        Meguru.INSTANCE.getAltManager().getUtils().loginWithString(generator.generate(8), "", false);
                        Meguru.INSTANCE.getAltManager().getAltPanel().refreshAlts();
                        break;
                }
            });
        ...
        mc.getTextureManager().bindTexture(new ResourceLocation("meguru/mc.png"));
    ...
    public void mouseClicked(int mouseX, int mouseY, int button) {
        ...
        if (hoveringMicrosoft && button == 0) {
            ...
            Meguru.INSTANCE.getAltManager().getUtils().microsoftLoginAsync(email, password);
            resetTextFields();
        }
