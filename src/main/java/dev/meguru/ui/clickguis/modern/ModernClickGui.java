package dev.meguru.ui.clickguis.modern;
...
import dev.meguru.ui.clickguis.modern.components.CategoryButton;
import dev.meguru.ui.clickguis.modern.components.ClickCircle;
import dev.meguru.ui.clickguis.modern.components.Component;
import dev.meguru.ui.clickguis.modern.components.ModuleRect;
...
public class ModernClickGui extends GuiScreen {
    ...
    public void initGui() {
        ...
        ClickGUIMod clickMod = Meguru.INSTANCE.getModuleCollection().getModule(ClickGUIMod.class);
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        if (ModuleCollection.reloadModules || moduleRects == null) {
            ...
            for (Category category : Category.values()) {
                ...
                for (Module module : Meguru.INSTANCE.getModuleCollection().getModulesInCategory(category)) {
        ...
        typing = modpanel.isTyping() || (Meguru.INSTANCE.getSideGui().isFocused() && Meguru.INSTANCE.getSideGui().isTyping()) || Meguru.INSTANCE.getSearchBar().isTyping();

        if (ClickGUIMod.walk.isEnabled() && !typing) {
            InventoryMove.updateStates();
        }

        boolean focusedConfigGui = Meguru.INSTANCE.getSideGui().isFocused();
        ...
        boolean searching = Meguru.INSTANCE.getSearchBar().isFocused();
        ...
        mc.getTextureManager().bindTexture(new ResourceLocation("meguru/modernlogo.png"));
        ...
        FontUtil.tenacityFont14.drawString(Meguru.VERSION, x + 41 + FontUtil.tenacityFont18.getStringWidth("meguru") + xAdjust, y + 15.5f,
                new Color(98, 98, 98));
    ...
    public List<ModuleRect> getModuleRects(Category category) {
        ...
        String search = Meguru.INSTANCE.getSearchBar().getSearchField().getText();
