...
import dev.meguru.intent.cloud.Cloud;
import dev.meguru.ui.Screen;
import dev.meguru.ui.altmanager.panels.LoginPanel;
import dev.meguru.ui.mainmenu.particles.ParticleEngine;
...
public class CustomMainMenu extends GuiScreen {
    ...
    private final ResourceLocation backgroundResource = new ResourceLocation("meguru/mainMenu/funny.png");
    private final ResourceLocation blurredRect = new ResourceLocation("meguru/mainMenu/rect-test.png");
    private final ResourceLocation hoverCircle = new ResourceLocation("meguru/mainMenu/hover-circle.png");
    ...
    public void initGui() {
        if (!firstInit) {
            ...
            if (Util.getOSType() == Util.EnumOS.WINDOWS) {
                Meguru.INSTANCE.setDiscordRPC(new DiscordRPC());
            }
            firstInit = true;
        }
        ...
        if (mc.gameSettings.guiScale != 2) {
            Meguru.prevGuiScale = mc.gameSettings.guiScale;
            Meguru.updateGuiScale = true;
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        Meguru.INSTANCE.videoRenderer.render(width, height);
...
//
        ...
        tenacityFont32.drawString(Meguru.VERSION, width / 2f + tenacityBoldFont80.getStringWidth("Meguru") / 2f - (tenacityFont32.getStringWidth(Meguru.VERSION) / 2f), height / 2f - 113, textcolor);
        if (Meguru.is0721){
            tenacityFont18.drawCenteredString("by 0d00", width / 2f, height / 2f - 68 + 5, textcolor);
        }else {
            tenacityFont18.drawCenteredString("by Sink", width / 2f, height / 2f - 68 + 5, textcolor);
        }
            ...
            button.clickAction = () -> {
                switch (button.text) {
                    ...
                    case "Multiplayer":
                        ...
                        break;
                    case "Alt Manager":
                        mc.displayGuiScreen(Meguru.INSTANCE.getAltManager());
                        break;
                    case "Settings":
    ...
    public void onGuiClosed() {
        if (Meguru.updateGuiScale) {
            mc.gameSettings.guiScale = Meguru.prevGuiScale;
            Meguru.updateGuiScale = false;
        }
    }
