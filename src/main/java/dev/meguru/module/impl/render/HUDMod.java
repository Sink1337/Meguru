...
import dev.meguru.module.impl.player.InvManager;
...
import dev.meguru.utils.misc.RomanNumeralUtils;
...
public class HUDMod extends Module {
    ...
    public void onShaderEvent(ShaderEvent e) {
        ...
        if (e.isBloom()) {
            ...
            String intentInfo = Meguru.INSTANCE.getIntentAccount().username;
            float WH = 110 / 2f;
            switch (watermarkMode.getMode()) {
                case "Moon":
                    mc.getTextureManager().bindTexture(new ResourceLocation("meguru/Moonlogo.png"));
                ...
                case "Logo":
                    ...
                    GradientUtil.applyGradientCornerLR(27, 23, WH - 28, WH - 28, 1, clientColors.getSecond(), clientColors.getFirst(), () -> {
                        mc.getTextureManager().bindTexture(new ResourceLocation("meguru/watermarkBack.png"));
                        Gui.drawModalRectWithCustomSizedTexture(7, 7, 0, 0, WH, WH, WH, WH);
                    });
                    break;
                case "Tenacity":
                    float xVal = 6f;
                    float yVal = 6f;
                    float versionWidth = FontUtil.tenacityFont16.getStringWidth(Meguru.INSTANCE.getVersion());
                    ...
                    GradientUtil.applyGradientHorizontal(xVal, yVal, width, 20, 1, clientColors.getFirst(), clientColors.getSecond(), () -> {
                        ...
                        if (version) {
                            FontUtil.tenacityFont16.drawString(Meguru.INSTANCE.getVersion(), versionX, yVal, 0);
                        }
                    });
    ...
    public void onRender2DEvent(Render2DEvent e) {
        ...
        Pair<Color, Color> clientColors = getClientColors();
        String name = Meguru.NAME;
        PostProcessing postProcessing = Meguru.INSTANCE.getModuleCollection().getModule(PostProcessing.class);
        ...
        version = name.equalsIgnoreCase(Meguru.NAME);
        ...
        switch (watermarkMode.getMode()) {
            case "Logo":
                ...
                RenderUtil.color(-1);
                GLUtil.startBlend();
                mc.getTextureManager().bindTexture(new ResourceLocation("meguru/watermarkT.png"));
                Gui.drawModalRectWithCustomSizedTexture(7, 7, 0, 0, WH, WH, WH, WH);
            ...
            case "Tenacity":
                ...
                GradientUtil.applyGradientHorizontal(xVal + spacing, yVal + spacing, width + spacing,
                        20, 1, darkerColors.getFirst(), darkerColors.getSecond(), () -> {
                            ...
                            if (version) {
                                FontUtil.tenacityFont16.drawString(Meguru.INSTANCE.getVersion(), versionX + (spacing / 2f), yVal + (spacing / 2f), 0);
                            }
                        });
            ...
            case "Exhi1":
                ...
                if (watermarkCustomization.getSetting("Protocol Version").isEnabled()) {
                    ComparableProtocolVersion currentVersion = ViaLoadingBase.getInstance().getTargetVersion();
                    stringBuilder.append(" [§f").append(currentVersion.getName()).append("§7]");
                }
                if (watermarkCustomization.getSetting("Client Version").isEnabled()) {
                    stringBuilder.append(" [§f").append(Meguru.VERSION).append("§7]");
                }
            ...
            case "Exhi2" :
                ...
                if (watermarkCustomization.getSetting("Protocol Version").isEnabled()) {
                    ComparableProtocolVersion currentVersion = ViaLoadingBase.getInstance().getTargetVersion();
                    stringBuilder1.append(" [§f").append(currentVersion.getName()).append("§7]");
                }
                if (watermarkCustomization.getSetting("Client Version").isEnabled()) {
                    stringBuilder1.append(" [§f").append(Meguru.VERSION).append("§7]");
                }
                if (watermarkCustomization.getSetting("Time").isEnabled()) {
    ...
    private void drawBottomRight() {
        ...
        if (infoCustomization.isEnabled("Show Bottom Right Text")) {
            String text = Meguru.VERSION + " - " + (customFont.isEnabled() ? "" : "§l") + Meguru.RELEASE.getName() + "§r";

            text += " - " + Meguru.INSTANCE.getIntentAccount().username;
            text += " (" + Meguru.INSTANCE.getIntentAccount().client_uid + ")";
    ...
    public static ModeSetting getCustomFontMode() {
        return ((HUDMod) Meguru.INSTANCE.getModuleCollection().getModule(HUDMod.class)).customFontMode;
    }
    ...
    public static void updateButtonStatus() {
        for (ModuleButton mb : ModuleButton.values()) {
            mb.getButton().enabled = Meguru.INSTANCE.getModuleCollection().getModule(mb.getModule()).isEnabled();
        }
    }
    ...
    public static void handleActionPerformed(GuiButton button) {
        for (ModuleButton mb : ModuleButton.values()) {
            if (mb.getButton() == button) {
                Module m = Meguru.INSTANCE.getModuleCollection().getModule(mb.getModule());
                if (m.isEnabled()) {
                    m.toggle();
                }
                break;
