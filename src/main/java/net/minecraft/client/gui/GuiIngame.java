...
public class GuiIngame extends Gui implements Utils {
    ...
    {
        if (HUDMod.customStatsRender.isEnabled()) {
    ...
    private void renderPlayerStats(ScaledResolution scaledRes)
    {
        if (HUDMod.customStatsRender.isEnabled()) {
            if (this.mc.getRenderViewEntity() instanceof EntityPlayer) {
                ...
                saturationAnim.animate(entityplayer.getFoodStats().getSaturationLevel() / 20f, 18);
        ...
        if (HUDMod.customStatsRender.isEnabled()) {
            ...
            if (mc.playerController.gameIsSurvivalOrAdventure()) {
                if (mc.thePlayer.getTotalArmorValue() > 0)
                    RoundedUtil.drawRound(halfWidth - 91, j2, 85, 8, 2, Color.BLACK);
